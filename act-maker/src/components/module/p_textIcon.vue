  ...
  methods: {
        ...
        for(var i = 0; i < that.txtArr.length; i++){
          ...
          if(element.insert != '^'){
            ...
            if(element.attributes.align == 'center'){
              ...
              ctx.textAlign='right'
              x =  width - staticC
              x0 = width > maxWidth ? width - maxWidth: 0
              x1 = width > maxWidth ? width - staticC : 0
              ox = x1 - r
            }else {
              ctx.textAlign='left'
              x = staticC
          ...
          if(element.insert == '^'){
            ...
            var underlineArr = []
            var boldArr = []
            for (var j = 0; j < element.insert.length; j++) {
              var preItem = j > 0 ? that.wordMessageArr[j-1] : undefined
              var item = that.wordMessageArr[j] ? that.wordMessageArr[j] : undefined 
              var itemWidth = item ? item.width : 0
              var strikeX0 = 0
              var strikeX1 = 0
              var underlineX0 = 0
              var underlineX1 = 0
              ...
              console.log(preItem && preItem.str)
              if(preItem){
                pointer += itemWidth
                if(item){
                  if(preItem.strike){
                    if(!item.strike){
                      strikeX1 = pointer
                      strikeArr.push(strikeX1)
                      if(item.underline){
                        underlineX0 = pointer
                        underlineArr.push(underlineX0)
                      }
                      if(item.bold){
                        boldX0 = pointer
                        boldArr.push(boldX0)
                  ...
                  if(preItem.underline){
                    if(!item.underline){
                      underlineX1 = pointer
                      underlineArr.push(underlineX1)
                      if(item.strike){
                        strikeX0 = pointer
                        strikeArr.push(strikeX0)
                      }
                      if(item.bold){
                        boldX0 = pointer
                        boldArr.push(boldX0)
                  ...
                  if(!preItem.strike && !preItem.underline && !preItem.bold){
                    if(item.strike){
                      strikeX0 = pointer
                      strikeArr.push(strikeX0)
                    }
            ...
            var lastWordMessage = that.wordMessageArr[element.insert.length-1]
            pointer += that.wordMessageArr[element.insert.length-1].width
            if(lastWordMessage.strike){
              strikeArr.push(pointer)
            }
            if(lastWordMessage.underline){
              underlineArr.push(pointer)
            }
            if(lastWordMessage.bold){
              boldArr.push(pointer)
            }

            console.log('```特殊标记数据')
            console.log(strikeArr)
            console.log(underlineArr)
            console.log(boldArr)
            that.wordMessageArr = that.wordMessageArr.slice( element.insert.length )


            if(strikeArr.length > 0){
              var tplstrike = []
              for(var l = 0; l < strikeArr.length; l++){
                tplstrike.push(strikeArr[l])
                if(tplstrike.length >= 2){
                  ctx.save()
                  ctx.moveTo (tplstrike[0] + x0,that.mark_size.height - (mParams.font.lineHeight * fontSize)/2)
                  ctx.lineTo (tplstrike[1] + x0,that.mark_size.height - (mParams.font.lineHeight * fontSize)/2)
                  ctx.lineWidth = 1;
                  ctx.strokeStyle = mParams.font.color
                  ctx.stroke()
                  ctx.restore()
            ...
            if(underlineArr.length > 0){
              var tplUnderlineArr = []
              for(var l = 0; l < underlineArr.length; l++){
                tplUnderlineArr.push(underlineArr[l])
                if(tplUnderlineArr.length >= 2){
                  ctx.save()
                  ctx.moveTo (underlineArr[0] + x0,that.mark_size.height)
                  ctx.lineTo (underlineArr[1] + x0,that.mark_size.height)
                  ctx.lineWidth = 1;
                  ctx.strokeStyle = mParams.font.color
                  ctx.stroke()
                  ctx.restore()
    ...
    formate: function(arr, ctx, staticC){
      ...
      for(var i = 0; i < txtArr.length; i++){
        var preElement = i > 0 ? txtArr[i-1] : undefined
        ...
        if(inserts != '^') {
          for(var j = 0; j < inserts.length; j++){
            ...
            var insertAttributes = inserts[j].attributes
            ...
            for(var k = 0; k < insertArr.length; k++){
              ...
              if(insertAttributes){
                word = {
                  strike: insertAttributes.strike,
                  underline: insertAttributes.underline,
                  bold: insertAttributes.bold,
                  width: this._ctx.measureText(insertArr[k]).width,
                  str: insertArr[k]
                }
              }else{
                word = {
      ...
      this.isDebug && console.log('```文字信息')
      // this.isDebug && console.log(this.wordMessageArr)
      // 新尝试 E!
